...
SAMPLE.ID = "endo_ovar_metastasis_37EACL-ovarian_reference"

## Part 1: scRNA-seq processing before doublet removal ####
################################################################################
source_dir <- "/mnt/Data/Vadim/POIAZ/Donagh/Ov_Endo_SingleCell/"

## Load the RNA dataset (Patient 1) ####
counts <- ReadMtx(mtx = paste0(source_dir,inputRNA[6]), cells = paste0(source_dir,barcodefiles[6]),
                  features = paste0(source_dir,featurefiles[6]))
...
counts <- ReadMtx(mtx = inputRNA[6], cells = barcodefiles[6], features = featurefiles[6])
...
rna.df$Sample <- rna@meta.data$SingleR.endo
...
rna.df$cluster <- rna@meta.data$postdoublet.idents

ggplot(rna.df,aes(x = UMAP_1,y=UMAP_2,color = cluster))+
  geom_point(size = .1)+
  theme_classic()+
...
ggsave("Sample_RNA.pdf",width = 8,height = 7)
